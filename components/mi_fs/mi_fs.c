...
void mi_fs_init(){   
    ...
    if (!logger_mutex) logger_mutex = xSemaphoreCreateMutex();
...
Logger read_logger_from_json() {
    ...
    char *json_str = read_json(logger_path);
    if (!json_str) {
...
void save_logger_to_json(const Logger *logger) {
    ...
    save_json(logger_path, json_str);
    free(json_str);
    cJSON_Delete(root);
}
...
void store_logger_event(Logger *logger, const char *event) {
    xSemaphoreTake(logger_mutex, portMAX_DELAY);
    char timestamp[32];
    mi_ntp_time_get_str(timestamp, sizeof(timestamp));
    char event_with_time[64];
    snprintf(event_with_time, sizeof(event_with_time), "%s|%s", event, timestamp);
    ESP_LOGI(TAG, "Storing logger event: %s", event_with_time);
    if (logger->pointer >= 20) {
        logger->pointer = 0;
    }
    strncpy(logger->events[logger->pointer], event_with_time, sizeof(logger->events[0]) - 1);
    logger->events[logger->pointer][sizeof(logger->events[0]) - 1] = '\0';
    logger->pointer++;
    save_logger_to_json(logger);
    xSemaphoreGive(logger_mutex);
}
...
void save_config_to_json(const Config *config) {
    ...
    char *json_str = cJSON_PrintUnformatted(root);
    save_json("/littlefs/config.json", json_str);
    free(json_str);
    cJSON_Delete(root);
    xSemaphoreGive(config_mutex);
}
